...
class Application(Frame):
    ...
    def createGUI(self):
        ...
        for obj in self.object:
            ...
            Scale(frame1, from_=-areasize / 2.0, to=areasize / 2.0, resolution=0.01, orient=HORIZONTAL,
                  variable=obj.x, width=scalewidth, command=obj.updateStateXYZHPR).grid(sticky=EW, row=row, column=1,
                                                                                        padx=padx)

            row += 1
            Label(frame1, text="y").grid(sticky=SE, row=row, column=0, padx=padx)
            Scale(frame1, from_=-areasize / 2.0, to=areasize / 2.0, resolution=0.01, orient=HORIZONTAL,
                  variable=obj.y, width=scalewidth, command=obj.updateStateXYZHPR).grid(sticky=EW, row=row, column=1,
                                                                                        padx=padx)
            ...
            Scale(frame1, from_=-1.0, to=1.0, resolution=0.01, orient=HORIZONTAL, variable=obj.wheel_angle,
                  width=scalewidth, command=obj.updateStateXYZHPR).grid(sticky=EW, row=row, column=1, padx=padx)

            row += 1
            Checkbutton(frame1, text="dead reckoning", variable=obj.dead_reckon, command=obj.updateStateXYZHPR,
                        onvalue=1, offvalue=0).grid(sticky=SW, row=row, column=1, padx=padx, pady=5)
            ...
            Scale(frame2, from_=-areasize / 2.0, to=areasize / 2.0, resolution=0.01, orient=HORIZONTAL,
                  variable=obj.x, width=scalewidth, command=obj.updateStateXYH).grid(sticky=EW, row=row, column=1,
                                                                                      padx=padx)

            row += 1
            Label(frame2, text="y").grid(sticky=SE, row=row, column=0, padx=padx)
            Scale(frame2, from_=-areasize / 2.0, to=areasize / 2.0, resolution=0.01, orient=HORIZONTAL,
                  variable=obj.y, width=scalewidth, command=obj.updateStateXYH).grid(sticky=EW, row=row, column=1,
                                                                                      padx=padx)
            ...
            Scale(frame2, from_=-1.0, to=1.0, resolution=0.01, orient=HORIZONTAL, variable=obj.wheel_angle,
                  width=scalewidth, command=obj.updateStateXYH).grid(sticky=EW, row=row, column=1, padx=padx)

            row += 1
            Checkbutton(frame2, text="dead reckoning", variable=obj.dead_reckon, command=obj.updateStateXYH,
                        onvalue=1, offvalue=0).grid(sticky=SW, row=row, column=1, padx=padx, pady=5)
            ...
            Scale(frame3, from_=-1.0, to=1.0, resolution=0.01, orient=HORIZONTAL, variable=obj.wheel_angle,
                  width=scalewidth, command=obj.updateStateH).grid(sticky=EW, row=row, column=1, padx=padx)

            row += 1
            Checkbutton(frame3, text="dead reckoning", variable=obj.dead_reckon, command=obj.updateStateH,
                        onvalue=1, offvalue=0).grid(sticky=SW, row=row, column=1, padx=padx, pady=5)
            ...
            Scale(frame4, from_=-1.0, to=1.0, resolution=0.01, orient=HORIZONTAL, variable=obj.wheel_angle,
                  width=scalewidth, command=obj.updateDriverInput).grid(sticky=EW, row=row, column=1, padx=padx)
            ...
            Entry(bottom_frame, textvariable=obj.inputModeText, state='disabled').grid(sticky=E, row=row, column=1,
                                                                                       padx=padx)

        Button(self.master, text="Quit", width=10, command=self.quit).pack(side=RIGHT)

        Checkbutton(self.master, text='continuous mode', variable=self.continuous, command=self.updateContinuousMode,
